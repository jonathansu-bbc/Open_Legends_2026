...
PID defendGoalPID(GOAL_TRACK_P, INTEGRAL, GOAL_TRACK_D, DEFEND_GOAL_TRACK_MAX);
PID defendHorizontalPID(HORIZONTAL_P, INTEGRAL, HORIZONTAL_D, HORIZONTAL_MAX);
PID defendVerticalPID(VERTICAL_P, INTEGRAL, VERTICAL_D, VERTICAL_MAX);
...
void Defender::update(const LSData& lsData, const CameraData& cameraData, const OrbitData& orbitData, float heading, float compassRot) {
    ...
    bool surge = cameraData.ball && (cameraData.ballDist < DEFEND_BALL_DISTANCE) && (cameraData.defendGoalDist < DEFEND_SURGE_DISTANCE) && (angleDiff(cameraData.ballAngle, 0.0f) < 30.0f);
